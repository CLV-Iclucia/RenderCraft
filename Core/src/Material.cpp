...
Vec3 Metal::BxDF(const Vec3 &wi, const Vec3 &wo, const Vec2 &uv) const {
  ...
  Vec3 ret = 0.25 * Fresnel(std::min(dot(H, wo), 1.0)) *
             surface->NormalDistribution(cosThetaH, uv) / cosThetaO *
             surface->ShadowMasking(cosThetaI, cosThetaO, uv) / cosThetaI;
  return ret;
}
...
Vec3 Dieletrics::sample(const Vec3 &wo, Real *pdf_inv, const Vec2 &uv) const {
...
Vec3 Dieletrics::BxDF(const Vec3 &wi, const Vec3 &wo, const Vec2 &uv) const {
  bool inside = wo[2] < 0.0;
  // TODO: fix this
 // Real eta = inside ? etaB / etaA : etaA / etaB; // eta = etaI / etaO
  ...
  if (cosThetaI >= 0.0) {
    ...
    Real cosThetaH = std::max(H[2], 0.0);
    Vec3 ret = 0.25 * Fr * color->eval(uv) *
               surface->NormalDistribution(cosThetaH, uv) / cosThetaO *
               surface->ShadowMasking(cosThetaI, cosThetaO, uv) / cosThetaI;
    return ret;
  } else {
    ...
    Vec3 ret = (1.0 - Fr) * std::abs(HdotWi) / cosThetaI * HdotWo / cosThetaO *
               surface->NormalDistribution(cosThetaH, uv) * color->eval(uv) /
               (tmp * tmp) * surface->ShadowMasking(cosThetaI, cosThetaO, uv);
    return ret;
  }
}
} // namespace rdcraft