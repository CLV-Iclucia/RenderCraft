...
inline Real xFit_1931(Real wavelength) {
  Real t1 = (wavelength - Real(442.0)) * ((wavelength < Real(442.0))
                                            ? Real(0.0624)
                                            : Real(0.0374));
  Real t2 = (wavelength - Real(599.8)) * ((wavelength < Real(599.8))
                                            ? Real(0.0264)
                                            : Real(0.0323));
...
class AdaptiveSpectrum {
  ...
  public:
    ...
    explicit AdaptiveSpectrum(const std::vector<std::tuple<int, Real>>& distrib)
      : distrib(distrib) {
    }
    Vec3 toXYZ() const {
      Real last_lambda = 0;
      Vec3 xyz;
      for (const auto& [lambda, value] : distrib) {
...
inline Spectrum toXYZ(const std::vector<std::tuple<int, Real>>& spec) {
  ...
  Real last_lambda{};
  Vec3 xyz{};
  for (const auto& [lambda, value] : spec) {
    auto coeff = XYZCoeff(lambda);
    xyz += (lambda - last_lambda) * coeff * value;
    last_lambda = lambda;
  }
