...
class GhostArray3D {
  public:
    ...
    T& operator()(const Vec3i& idx) {
      return m_data[(idx.x + Padding) +
                    (idx.y + Padding) * (m_width + 2 * Padding) +
                    (idx.z + Padding) * (m_width + 2 * Padding) *
                    (m_height + 2 * Padding)];
    }
    ...
    void fill(const T& non_padding_value, const T& padding_value) {
      for (int i = 0; i < m_width + 2 * Padding; i++) {
        for (int j = 0; j < m_height + 2 * Padding; j++) {
          for (int k = 0; k < m_depth + 2 * Padding; k++) {
            if (i < Padding || i >= m_width + Padding ||
                j < Padding || j >= m_height + Padding || k < Padding || k >=
                m_depth + Padding) {
